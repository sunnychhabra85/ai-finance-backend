...
if ! npx prisma migrate deploy --schema=apps/upload-service/prisma/schema.prisma 2>&1 | tee /tmp/migration.log; then
    if grep -q "P3005" /tmp/migration.log; then
        echo "Database not empty, using db push to sync schema..."
        npx prisma db push --schema=apps/upload-service/prisma/schema.prisma --skip-generate --accept-data-loss
    else
